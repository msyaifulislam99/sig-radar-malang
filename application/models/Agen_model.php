...
class Agen_model extends CI_Model {
  ...
  var $table = 'tbl_agen';
  var $column_order = array('id_agen', 'nama_agen','alamat_agen'); //set column field database for datatable orderable
  var $column_search = array('id_agen','nama_agen','alamat_agen'); //set column field database for datatable searchable 
  var $order = array('id_agen' => 'asc'); // default order 
  public $urutanagen = 0;
   ...
   */
  public function cari($id_agen)
  {
      return $this->db->select('*,X(lokasi) AS lattitude, Y(lokasi) AS longitude')
          ->where('id_agen', $id_agen)
          ->limit(1)
          ->get($this->table)
          ->row();

      // return $this->db->where('id_agen', $id_agen)
      //     ->limit(1)
      //     ->get($this->table)
      //     ->row();
  }
